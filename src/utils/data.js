...
/**
 * 修复admin菜单判断的问题（与新老系统并行兼容工具）
 * @param {*} menus 
 * @param {*} userInfo 
 */
export const fixMenu = function (menus, userInfo) {
  let menuNum = -1
  menus.every((item, index) => {
    let res = item.menuCode === 'ECS_JCXX'
    if (res) {
      menuNum = index
    }
    return !res
  ...
  if (menuNum !== -1) {
    let innerMenus = menus[menuNum].children
    let realJCXXMenu = []
    innerMenus.forEach((item) => {
      if (settings.onlyAdminMenu.indexOf(item.menuCode) !== -1) {
        if (userInfo.userLoginId === 'ECS_Admin') {
          realJCXXMenu.push(item)
        }
      } else {
        realJCXXMenu.push(item)
      }
    })
    menus[menuNum].children = realJCXXMenu
  }
  return menus
}
