  ...
  <div>
    <ep-model title="卡口验放凭证表体" v-model="model" width="1200px" :wrap-close="false">
      <ep-form ref="portReleaseDt" :form="portReleaseDt" name-width="168px">
        <ep-row :gutter="7">
          ...
          <ep-col :col="8">
            <ep-form-item attr="classifyType" label="料件 /成品标志" v-if='classify'>
          ...
           <ep-col :col="8">
               <ep-form-item attr="goodsType" label="货物类型" v-if='classify'>
    ...
    <div class="block">
      <ep-table ref="table" border :height="400" :data="tableData" @selection-change="handleSelect" @row-dblclick="select1Info" :head-color="headColor">
        ...
        <ep-table-item column="dclUnitcd" title="申报计量单位" width="130" :formatter="dclUnitcdTranslate"></ep-table-item>
        ...
        <ep-table-item column="classifyType" width="150" title='料件/成品标志' :formatter="classifyType" v-if='classify'></ep-table-item>
        <ep-table-item column="goodsType" width="150" title='货物类型' :formatter="goodsType" v-if='classify'></ep-table-item>
      </ep-table>
    </div>
    <div class="block">
...
export default {
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      ...
      this.makePaging(this.ep_page.size, this.ep_page.page, this.dtFormBody);
    },
    // 每页条数
