<template>
  <div>
    <ep-model 
      ...
      v-model="isShowModel"
      width="1200px">
      <ep-form class="demo-form" ref="woOutputModel" :form="woOutputModel" name-width="168px">
        <ep-row :gutter="10">
          <ep-col :col="8">
            <ep-form-item label="产出类型" attr="itemType">
              <ep-select v-model="woOutputModel.itemType" name="itemType" disabled>
          ...
          <ep-col :col="8">
            <ep-form-item label="料号" attr="itemNo">
              <ep-input v-model="woOutputModel.itemNo" name="itemNo" disabled></ep-input>
            </ep-form-item>
          </ep-col>
          <ep-col :col="8">
            <ep-form-item label="数量" attr="qty">
              <ep-input v-model="woOutputModel.qty" name="qty" disabled></ep-input>
            </ep-form-item>
          </ep-col>
          <ep-col :col="8">
            <ep-form-item label="申报计量单位" attr="unit">
              <ep-select
                v-model="woOutputModel.unit"
                name="unit"
                filterable
                disabled
              >
    ...
    <div class="ep-card card-margin relative">
      <div class="card-body">
        <div class="block">
          <ep-table ref="woOutputTable" :height="400" :data="tableData" head-color  @row-dblclick="detailInfo">
            <ep-table-item column="itemType" title="产出类型" width="200" :formatter="takeTypeFormatter"></ep-table-item>
...
export default {
  ...
  created() {},
  mounted() {
    this.makePaging(10, 1, this.woOutputTable)
  },

  methods: {
    // 表体点击查询
    searchBtn(){
      let filterData = this.filterData(this.searchForm,this.woOutputTable)
    ...
    handleSizeChange(val){
      ...
      this.makePaging(this.ep_page.size,this.ep_page.page, this.filterData(this.searchForm,this.woOutputTable))
    },
