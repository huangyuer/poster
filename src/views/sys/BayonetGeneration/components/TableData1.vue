        ...
        <div class="block">
          <div style="background:#fff;">
            <div style="display:flex;justify-content: flex-end;align-items:center;padding-right:20px">
              ...
              <!-- <ep-switch on-text="横向" off-text="竖向"></ep-switch> -->
            </div>
            <div class="drag-box" :class="format?'':'drag-box1'">
              <div class="drag-box-item1">
                <div>
                  ...
                  <ep-table v-if="format" ref="table1" :height="format?400:200" :data="ep_data" :head-color="true" :loading="loading" @row-click="headerClick" @selection-change="selectChange"
                    @row-dblclick="detailInfo" :row-class="setRowClass">
                    <ep-table-item type="select"></ep-table-item>
                    <ep-table-item column="billNo" title="关联单证编号">
                      <template slot-scope="props">
                        <a>{{ props.row.billNo }}</a>
                      </template>
                    </ep-table-item>
                  </ep-table>
                  <ep-table v-else ref="table1" :height="format?400:200" :data="ep_data" :head-color="true" :loading="loading" @row-click="headerClick" @selection-change="selectChange"
                    @row-dblclick="detailInfo" :row-class="setRowClass">
                    <ep-table-item type="select"></ep-table-item>
                    <ep-table-item column="billNo" title="关联单证编号">
                      <template slot-scope="props">
              ...
              <div class="drag-box-item2">
                ...
                <div class="item-title">进境径予自主声明详情</div>
            ...
            <ep-model title="表体编辑" v-model="infoModel" width="500px" :wrap-close="false">
              <ep-form ref="infoForm" :form="infoForm" name-width="108px">
                ...
                <div style="text-align:center">
                  ...
                  <ep-button type="primary" size="small" btn-click-disabled @click="makeSuer">确定</ep-button>
                </div>
              </ep-form>
            </ep-model>
...
export default {
  ...
  data() {
    return {
      format: this.formats,
      loading: false,
      listApi: 'queryBillDirDetail', // 搜索，取api.json里面的key值
  ...
  methods: {
    ...
    Drag() {
      ...
      const sortable = Sortable.create(el, {
        ...
        onEnd: evt => {
          ...
          if (this.calculation(el1, box, x2, y)) {
            ...
            if (this.ep_data[evt.oldIndex].billDirDt.length == 0) {
              this.$pop({
                type: 'danger',
                message: '该表体为空!'
              })
            ...
            this.dataList = []
            this.dataList.push({
              rltNo: this.ep_data[evt.oldIndex].billNo,
              rltTbTypecd: this.judgeForm.rtlTypecd == '3' ? '1' : '2'
            })
            this.doing = this.ep_data[evt.oldIndex].billDirDt
          ...
          if (this.calculation(el2, box1, x2, y)) {
            ...
            // this.done = [...this.ep_data[evt.oldIndex].billDirDt]
            this.done = [...this.done, ...this.ep_data[evt.oldIndex].billDirDt]
            this.$set(this.ep_data[evt.oldIndex], 'Highlight', true)
            setTimeout(() => {
              this.sign()
              this.selectUSDCol()
            }, 10)
            //   this.ep_data[evt.oldIndex].Highlight = true

            //   var ocuritem = this.ep_data.pop()
            //   this.ep_data.push(ocuritem)
            //   let dom = evt.item
            //   dom.className = 'light-success'
      ...
      const sortable2 = Sortable.create(el2, {
        ...
        onEnd: evt => {
          ...
          y = evt.originalEvent.clientY
          w = el2.offsetWidth
          //   !(x1 - x2 < w / 4 || x1 - x2 > (x1 - x2) / 4 + el1.offsetWidth)
          if (this.calculation(el1, box, x2, y)) {
            if (
              this.judgeForm.bindTypecd == '1' ||
              this.judgeForm.bindTypecd == '2'
    ...
    generateDrop(val) {
      ...
      // this.Listdata2 = this.list
      // this.Listdata2.totalGrossWt = ''
      // this.Listdata2.containerWt = ''
      // this.Listdata2.vehicleFrameWt = ''
      // this.Listdata2.vehicleWt = ''
      // this.Listdata2.totalNetWt = ''
      this.Listdata2.whRecPreentNo = this.list.billNo //出入库编号
    ...
    hearderDelete() {
      ...
      this.ep_data.forEach((element, index) => {
        selectData.forEach(item => {
          if (item.billNo == element.billNo && item.Highlight != true) {
            this.$set(this.ep_data[index], 'Highlight', true)
          }
        })
      })
      selectData.forEach(item => {
        item.billDirDt.forEach((items, index) => {
