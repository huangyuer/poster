            ...
            <div class="drag-box" :class="format?'':'drag-box1'">
              ...
              <div class="drag-box-item1">
                <div>
                  ...
                  <ep-table ref="table1" :height="format?400:200" :data="ep_data" :head-color="true" :loading="loading" @row-dblclick="detailInfo" :row-class="setRowClass">
                    <ep-table-item type="select"></ep-table-item>
                    <ep-table-item column="billExpNo" title="关联单证编号">
                      <template slot-scope="props">
                        <a>{{ props.row.billExpNo }}</a>
                      </template>
                    </ep-table-item>
                  </ep-table>
                </div>
              ...
              <div class="drag-box-item2">
                ...
                <div class="item-title">出境径予自主声明详情</div>
...
export default {
  ...
  data() {
    return {
      format: this.formats,
      loading: false,
      listApi: 'queryBillDirExpDetail', // 搜索，取api.json里面的key值
  ...
  methods: {
      ...
      const sortable = Sortable.create(el, {
        ...
        onEnd: evt => {
          ...
          if (this.calculation(el1, box, x2, y)) {
            ...
            if (this.ep_data[evt.oldIndex].billDirExpDt.length == 0) {
              this.$pop({
                type: 'danger',
                message: '该表体为空!'
              })
            ...
            this.dataList = []
            this.dataList.push({
              rltNo: this.ep_data[evt.oldIndex].billExpNo,
              rltTbTypecd: this.judgeForm.rtlTypecd == '3' ? '1' : '2'
            })
            this.doing = this.ep_data[evt.oldIndex].billDirExpDt
          ...
          if (this.calculation(el2, box1, x2, y)) {
            ...
            this.done = [
              ...this.done,
              ...this.ep_data[evt.oldIndex].billDirExpDt
            ]
            this.$set(this.ep_data[evt.oldIndex], 'Highlight', true)
            setTimeout(() => {
              this.sign()
    ...
    generateDrop(val) {
      ...
      this.Listdata2.whRecPreentNo = this.list.billExpNo //出入库编号
    ...
    hearderDelete() {
      ...
      this.ep_data.forEach((element, index) => {
        selectData.forEach(item => {
          if (item.billExpNo == element.billExpNo && item.Highlight != true) {
            this.$set(this.ep_data[index], 'Highlight', true)
          }
        })
      })
      selectData.forEach(item => {
        item.billDirExpDt.forEach((items, index) => {
          this.done.push(items)
        })
        this.dataList.push({
          rltNo: item.billExpNo,
          rltTbTypecd: this.judgeForm.rtlTypecd == '3' ? '1' : '2'
        })
      })
