  ...
  <div class="panel-main-content">
    <div class="contents-card card-margin">
      <div class="panel panel-default">
        <ep-tabs active="进出口货物状态表">
          <ep-tab-item name="进出口货物状态表">
            <bodyinfo :searchForm="searchForm" ref="searchForm"></bodyinfo>
          </ep-tab-item>

          <!-- 汇总的表体 -->
          <ep-tab-item name="进出口货物汇总表体">
            <div>
            ...
            <!-- <headerinfo :tableData="bodyForm"></headerinfo> -->
          </ep-tab-item>
          <!-- 操作记录部分 -->
          <!-- <ep-tab-item name="操作记录">
            <WholeOptStatus :tableData="operateData" :ModelType="'sum'"></WholeOptStatus>
          </ep-tab-item> -->
        </ep-tabs>
      </div>
    </div>
...
export default {
  ...
  name: "importBondedInfos",
  components: { bodyinfo, headerinfo },
  data() {
    return {
      bodyForm: [],
      tableData: [],
  ...
  created() {
    ...
    let formData = JSON.parse(sessionStorage.getItem("headerFrom"));
  ...
  methods: {
    ...
    againDate(id) {
      ...
      console.log("wwwwwwwwwwwwwwwwwwwww", data);
      this.$post("importBondedDetail", data).then(res => {
        this.totalcount = res.map.data.totalElements;
        this.tableDataCopy = res.map.data.content;
        // this.searchForm = res.map.data;
        // this.operateData = res.map.data.sumStatisticsStatus;
        // this.bodyForm = res.map.data.sumStatisticsDt;
        // console.log('')
        // this.$refs.bodyForm.makePaging(10, 1, this.bodyForm);
      });
    }
  }
};
