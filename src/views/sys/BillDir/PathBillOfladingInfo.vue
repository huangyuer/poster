  ...
  <div>
    <div class="panel-main-content">
      ...
      <div class="search-card contents-card card-margin">
        <wms-title title>
          <div class="panel panel-default">
            ...
            <ep-form ref="searchForm" :form="searchForm" name-width="138px">
                ...
                <ep-col :col="8" v-if="searchType">
                  <ep-form-item label="状态" attr="optStatus">
                    <ep-select v-model="searchForm.optStatus" name="optStatus">
                      ...
                      <ep-select-item index="3" label="入库成功"></ep-select-item>
              ...
              <ep-row :gutter="7">
                <ep-col :col="24">
                  <!-- <ep-button type="warning" size="small" icon="refresh" >重置</ep-button>
        ...
        <div class="card-body">
          ...
          <div class="block">
            <ep-table ref="table" border :height="400" :data="ep_data" :head-color="headColor" @selection-change="handleSelectionChange">
    ...
    <ep-model title="货物详情" v-model="model1" moveable>
      <div class="block" v-if="model1">
        <ep-table ref="table" :height="400" :data="ep_data2" :head-color="headColor" @selection-change="handleSelectionChange" border>
        ...
        <!--<div class="block">
          <ep-pager id="model11" right @size-change="handleSizeChange1" @change="handleCurrentChange1" :now-page="ep_page1.page" :page-size="ep_page1.size" :total-num="totalcount1"></ep-pager>
        </div>-->
        <CustomPager v-if="model3" @size-change="handleSizeChange1" @change="handleCurrentChange1" :ep_data="ep_data2" :nowPage="ep_page1.page" :pageSize="ep_page1.size"></CustomPager>
  ...
  methods: {
        ...
        this.$confirm({
          ...
          callback: (errorInfo) => {
            if (errorInfo) {
              ...
              let data = {};
              data["bscIds"] = bscIds;
              this.$post("deletePathBillOfladingInfo", data)
                .then((responseData) => {
                  this.refresh(true);
                  this.$pop({
                    type: "success",
    ...
    doModify(row, rowId) {
      ...
      if (selectData[0].optStatus === "5") {
        ...
        let data = {};
        data["id"] = id;
        this.$post("queryModifyGetById", data)
          .then((responseData) => {
            console.log(responseData);
            if ("1" === responseData.status) {
              //                //如果通过数据校验的话进行跳转
              this.$app.trigger("locate-tab", "billDirDtModify", id);
            } else {
              console.log(responseData);
              this.$pop({
                type: "danger",
    ...
    doInvalidConfirm() {
      ...
      //校验能否执行作废操作
      //真正执行作废
      this.$post("billDirDtInvalid", data)
        .then((responseData) => {
          //作废完成
          if ("1" === responseData.status) {
            this.$pop({
    ...
    doUpdate(row, rowId) {
      ...
      if (
        ...
        let data = {};
        data["id"] = id;
        this.$app.trigger("locate-tab", "billDirDtUpdate", id);
      } else {
        this.$pop({
          type: "danger",
          message:
            // '只有暂存、入库失败、审核拒绝、变更拒绝、变更暂存、作废暂存、作废拒绝、海关变更入库失败状态下的径予放行货物自主声明可以修改'
            "请选择其他状态下的径予放行货物自主声明进行修改",
        });
        return;
      }
    ...
    doDeclare(row, rowId) {
      ...
      if (
        ...
        if (selectData[0].hazardousConfirm != "2") {
          this.$pop({
            type: "danger",
            message: "危险品和危化品无法申报",
          });
        ...
        if (selectData[0].agreeConfirm != "1") {
          this.$pop({
            type: "danger",
            message: "未承诺货物符合径予放行条件的径予放行货物自主声明无法申报",
          });
          return;
        }
        if (selectData[0].involveConfirm != "0") {
          this.$pop({
            type: "danger",
            message: "涉证或涉检的径予放行货物自主声明无法申报",
          });
          return;
        }
        let ids = [];
    ...
    doPrint(row, rowId) {
      ...
      window.location.href =
        global.HOST + "/billDirBsc/billDirBscPDF?id=" + id + "&token=" + token;
    },
    detailInfo(event, row, index) {
      this.$app.trigger("locate-tab", "BillDirDtInfo", row.id);
    },
  },
  data() {
    return {
      uploadData: {
        businessType: "billDirBsc",
        sysCode: "lgsa",
      },
      counter: 0,
      tradeInfo: [],
      ...
      loading: false,
      listApi: "pathBillOfladingInfoGetList", // 搜索，取api.json里面的key值
