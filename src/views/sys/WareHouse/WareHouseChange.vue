...
export default {
  ...
  beforeDestroy() {
    ...
    bus.$off("passBodyChangeRe");
    bus.$off("deleteChangeRe");
    bus.$off("addsomeupdate");
  },
  mounted() {
  ...
  methods: {
    ...
    doSave() {
      ...
      this.$refs.formHeader.$refs["bodyForm"].validate(valid => {
        if (valid) {
          ...
          this.$post("whChange", this.headerForm)
            .then(res => {
    ...
    //               let response = await vm.$post("whChange", {
    //                 // id: [id]
    //                 bscIds: [id]
    //               });
