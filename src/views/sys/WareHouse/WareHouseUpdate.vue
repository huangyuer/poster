...
export default {
  ...
  created() {
    ...
    bus.$on("passBodyDtUpOut", data => {
     ...
     bus.$on("deleteDtDetailUpOut", data => {
  ...
  beforeDestroy() {
    ...
    bus.$off("passBodyDtUpOut");
    bus.$off("deleteDtDetailUpOut");
  },
  mounted() {
  ...
  methods: {
    ...
    againDate(id) {
      this.$post("wareHouseBscDetails", { id: parseInt(id) }).then(res => {
        ...
        this.rltWhRecNochangeState=this.headerForm.rltWhRecNo;
