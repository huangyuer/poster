...
import { delToken, getToken } from '../utils/oauth'
 ...
 * 
 * Settings:
 * @param {String} [settings.methods] [GET|POST] Request
...
function setParams (settings) {
  ...
  // Cache
  if (false === settings.cache) {
    let prefix = url.indexOf('?') === -1 ? '?' : '&'
    url += prefix + '_v=' + new Date().getTime()
  }
...
function checkStatus (response) {
  if (response.status >= 200 && response.status < 300) {
    ...
    if (response.status === 404) {
      ...
      Vue.prototype.$pop({
        type: 'danger',
        message: '您没有相关权限,请查看您的权限！',
        hasClose: true
      })
    }else if (response.status === 405) {
      Vue.prototype.$pop({
        type: 'danger',
        message: '您访问的频率太快了,请稍后再进行尝试！',
        hasClose: true
      })
      ...
      // localStorage.delLocalStorage("orgId");
      // localStorage.delLocalStorage("setOrgInfo");
      // store.commit('SETID',"")
