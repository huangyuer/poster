...
export default {
  ...
  methods: {
    ...
    initDataBus () {
        ...
        this.$app.on("add-new-tab",(link)=>{
            ...
            this.$router.push({path:menusList[0].link})
            // menusList.push(itemActive)
            setTimeout(() => {
  ...
  data () {
    ...
    let menuTabInstance = new menuTab(menuItems, _ => {
      if (this.$refs.frameTab) {
        this.$refs.frameTab.addProp()
      }
    })
    this.$store.dispatch('setMenuTabInstance', menuTabInstance)
    return {
      realKeepAliveInclude: [],
      canChangeTabFlag: true,
      menuTab:menuTabInstance
    }
  },
